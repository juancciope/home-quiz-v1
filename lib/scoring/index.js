...
function levelFromPct(pct) {
  console.log(`🔍 levelFromPct input: ${pct}`);
  if (pct >= 80) {
    console.log(`→ Returning "Core Focus" (${pct} >= 80)`);
    return 'Core Focus';
  }
  if (pct >= 45) {
    console.log(`→ Returning "Strategic Secondary" (${pct} >= 45)`);
    return 'Strategic Secondary';
  }
  console.log(`→ Returning "Noise" (${pct} < 45)`);
  return 'Noise';
}
...
function scoreUser(responses) {
  ...
  const levels = {};
  console.log('🔍 Calculating levels from absPct:', absPct);
  for (const p of PATHS) {
    const absPercent = absPct[p];
    const calculatedLevel = levelFromPct(absPercent);
    levels[p] = calculatedLevel;
    console.log(`🎯 Path ${p}: ${absPercent}% → "${calculatedLevel}"`);
  }
  console.log('🔍 Final levels object:', levels);
