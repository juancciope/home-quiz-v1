...
async function createSubscriptionWithAutomation(email, publicationId, automationId, additionalData = {}) {
  ...
  const payload = {
    email: email,
    reactivate_existing: true, // Reactivate if previously unsubscribed
    send_welcome_email: false, // Automation will handle welcome sequence
    double_opt_override: 'on',
    automation_ids: [automationId], // Enroll in automation after creation
