...
const HOMEQuizMVP = () => {
  ...
  return (
    <JourneyLayout masterStage={stage} resultStep={resultStep} onBack={handleBack} onNext={handleNext} currentScreen={currentScreen} questionIndex={questionIndex} mainRef={mainContentRef}>
...
const JourneyLayout = ({ children, masterStage, resultStep, onBack, onNext, currentScreen, questionIndex, mainRef }) => {
    const stages = [
    ...
    const activeSubNavItemRef = useRef(null);

    useEffect(() => {
        if (activeSubNavItemRef.current && subNavRef.current) {
            activeSubNavItemRef.current.scrollIntoView({
                behavior: 'smooth',
                block: 'nearest',
                inline: 'center'
            });
        ...
        <div className="h-[100svh] bg-gray-50 flex flex-col" style={{ fontFamily: 'Montserrat, sans-serif' }}>
            ...
            <header className="flex-shrink-0 bg-white/95 backdrop-blur-sm z-20 shadow-sm pt-3 pb-2">
                <div className="container max-w-5xl mx-auto px-4">
                    ...
                    {masterStage === 3 && (
                        <div ref={subNavRef} className="mt-2 flex justify-start sm:justify-center items-center border-t pt-2 overflow-x-auto no-scrollbar">
                            {[...Array(4)].map((_, i) => (
                                <div 
                                    key={i} 
                                    ref={resultStep === i + 1 ? activeSubNavItemRef : null}
                                    className={`text-xs font-semibold p-1 rounded transition-all duration-300 flex-shrink-0 mx-2 ${resultStep === i + 1 ? 'text-white' : 'text-gray-500'}`}
